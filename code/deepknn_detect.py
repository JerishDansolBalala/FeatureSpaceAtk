...
from cleverhans.picklable_model import MLP, Conv2D, ReLU, Flatten, Linear, Softmax, ResidualWithGroupNorm, GlobalAveragePool,  ResidualWithBatchNorm, PerImageStandardize
from cleverhand_more import ResidualWithInstanceNorm
...
def preprocess(x,y):
  return x.astype(np.float32),np.eye(10)[y].astype(np.float32)

class dknn_detection:
  def __init__(self,data,sess,atk_sym,x_sym,y_sym):
    self.x_train,self.y_train,self.x_test,self.y_test=data
    self.x_train, self.y_train = preprocess(self.x_train, self.y_train)
    self.x_test, self.y_test = preprocess(self.x_test, self.y_test)
    self.sess=sess
    self.atk_sym=atk_sym
    self.x_sym=x_sym
    #self.y_sym=y_sym
    self.y_sym = tf.placeholder(tf.float32, shape=(None, 10))
    self._flag_init_()

  def _flag_init_(self):
    ...
    tf.flags.DEFINE_integer('nb_epochs', 30, 'Number of epochs to train model')
    if VERSION==1:
  ...
  def build_model(self):
    ...
    self.loss = CrossEntropy(model, smoothing=0.)
    self.model=model
    self.preds=preds
  ...
  def pretrain(self,):
    ...
    def evaluate():
        acc = model_eval(self.sess, x, y, preds, x_test, y_test,
    ...
    train(self.sess, loss, x_train, y_train, evaluate=evaluate,
    ...
    def get_activations(data):
        ...
        for layer in layers:
            ...
            data_activations[layer] = batch_eval(self.sess, [x], [layer_sym], [data],
    ...
    if VERSION==1:
      ...
      layers = ['ReLU2', 'ResidualWithInstanceNorm4',
        'ResidualWithInstanceNorm6',
        'ResidualWithInstanceNorm8','logits']
    ...
    #self.fgsm = BasicIterativeMethod(model, sess=self.sess)
    #attack_params = {'eps': .03*255, 'clip_min': 0., 'clip_max': 1.*255}
    #attack_params = {'eps': .03, 'eps_iter': .03/10, 'clip_min': 0., 'clip_max': 1., 'nb_iter': 40}
    #y_argmax=tf.argmax(y,-1)
    #self.adv = self.fgsm.generate(x, y=y_argmax, **attack_params)
  ...
  def get_logits(self,x,y):
    ...
    wrong_logit = tf.reduce_max(
        logits * (1-label) - label * 1e7, axis=-1)
    true_logit = tf.reduce_sum(
        logits * label, axis=-1)
    loss = -tf.reduce_sum(tf.nn.relu(true_logit - wrong_logit + conf) )

    #loss = tf.losses.softmax_cross_entropy(label,logits)
    return logits, loss
      
  def test_acc(self,image,label):
    ...
    dknn_preds = self.dknn.fprop_np(image)
    #print(dknn_preds.shape)
    np_m=(np.mean(np.argmax(dknn_preds, axis=1) == np.argmax(label, axis=1)))

    return np_m

  #def test_fgsm(self,image,label):
  #    adv = self.sess.run(self.adv, feed_dict={self.x:image, self.y:label})
  #    dknn_preds = self.dknn.fprop_np(adv)
  #    np_m=(np.mean(np.argmax(dknn_preds, axis=1) == np.argmax(label, axis=1)))
  #    return np_m
