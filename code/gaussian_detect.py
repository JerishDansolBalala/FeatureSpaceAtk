...
with tf.Graph().as_default(), tf.Session(config=tf_config) as sess:
    ...
    if data_set == "cifar10":
        ...
        pgd_attack = LinfPGDAttack(classifier.xent, content, label, epsilon=8.0 , num_steps=10, step_size=2.0, random_start=True)
    elif data_set == "imagenet":
    ...
    for i in range(11,126):
        ...
        print("result normal:")
        _ , p_set_normal, p_det_normal = gaussdetect.detect(x_train_val, y_train_val, batch_size=BATCH_SIZE)
        print("result pgd:")
        _, p_set_pgd, p_det_pgd = gaussdetect.detect(x_train_perturbed, y_train_val, batch_size=BATCH_SIZE)    
        print("result adv:")
        _, p_set_adv, p_det_adv = gaussdetect.detect(x_train_style[0:], y_train_val[0:], batch_size=BATCH_SIZE)

        np_adv_image.append(x_train_style)
        np_benign_image.append(x_train_val)
        np_pgd_image.append(x_train_perturbed)
        np_label.append(y_train_val)
        np_pred_normal.append(p_set_normal)
        np_detection_normal.append(p_det_normal)

        np_pred_adv.append(p_set_adv)
        np_detection_adv.append(p_det_adv)

        np_pred_pgd.append(p_set_pgd)
        np_detection_pgd.append(p_det_pgd)

        if i % report_batch == 0:
            np_adv_image = np.concatenate(np_adv_image)
            np_benign_image = np.concatenate(np_benign_image)
            np_pgd_image = np.concatenate(np_pgd_image)
            np_label = np.concatenate(np_label)
            np_pred_normal = np.concatenate(np_pred_normal)
            np_detection_normal = np.concatenate(np_detection_normal)

            np_pred_adv = np.concatenate(np_pred_adv)
            np_detection_adv= np.concatenate(np_detection_adv)

            np_pred_pgd= np.concatenate(np_pred_pgd)
            np_detection_pgd = np.concatenate(np_detection_pgd)

            saved_dict = {"adv_image": np_adv_image, 
                        "benign_image": np_benign_image,
                          "pgd_image": np_pgd_image,
                        "pred_normal" : np_pred_normal,
                        "detect_normal": np_detection_normal,
                        "pred_adv": np_pred_adv,
                        "detection_adv": np_detection_adv,
