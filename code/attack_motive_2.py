...
for decoder_name in ["imagenet_shallowest", "imagenet_shallow", "imagenet"]:
    ...
    with tf.Graph().as_default(), tf.Session(config=tf_config) as sess:
        ...
        report_batch = 50
        aname1="nat"
        os.makedirs("motiveimg",exist_ok=True)
        with open(os.path.join("motiveimg","labels.txt"),"w") as f:
            for batch in range(1, motive_batch):
                step = batch
                x_batch, y_batch = prefetch[batch]
                out_img = []
        ...
        p_val = 1.00
        for aname1 in ["selfaug_mean",  "polygon", "selfaug_sigma", "pgd", "selfaugall"]:
