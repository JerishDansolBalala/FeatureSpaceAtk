    ...
    for batch in range(300000):
        ...
        if batch % 1000 ==0:
            for i in range(8):
                ...
                full_path = os.path.join(decoder_name+
                    "img%.2f" % style_weight, "mode%d"%mode, "%d" % step,  "%d.jpg" % i)
                os.makedirs(os.path.join(decoder_name+"img%.2f" %
                                         style_weight, "mode%d" % mode, "%d" % step), exist_ok=True)
        ...
        if batch % 100 == 0:
            ...
            elapsed_time = datetime.now() - start_time
            _content_loss, _adv_acc, _adv_loss, _loss, _l2_embed = sess.run([content_loss, adv_acc, adv_loss, loss, l2_embed],
                                                                  feed_dict=fdict)
