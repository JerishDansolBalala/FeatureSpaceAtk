...
with tf.Graph().as_default(), tf.Session(config=tf_config) as sess:
    ...
    if tid == 0:
        ...
        if data_set =="imagenet":
            dp = datapair(1000, batch_size=8, stack_num=1)
            f = True
            while f:
                x_batch, y_batch = get_data(sess)
                f = dp.feed_pair(x_batch, y_batch)
            print("data loaded")
            normal_arr = np.concatenate(dp.bucket)
        ...
        for i in range(bs):
            ...
            if TRAIN_DECODER:
                ...
                ### construct adv corresponding label
                concat_label = sess.run(normal_label, feed_dict=fd_adv)
                correspond_image_adv = []
                for k in range(BATCH_SIZE):
                    correspond_image_adv .append(normal_arr[k]) 
                correspond_image_adv=np.stack(correspond_image_adv)
            ...
            fd_correspond_adv = {content: correspond_image_adv}
            ...
            embed_correspond_adv = sess.run(stn.normalized,  feed_dict=fd_correspond_adv)

            embeds.append([embed_benign,embed_correspond_adv,embed_adv,embed_decoder,embed_pgd])
